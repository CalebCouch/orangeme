...
class QrPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    int middleSize = (matrix.width / 3.4).floor();  // Make the center square bigger
    ...
    for (int i = 0; i < matrix.width; i++) {
      for (int j = 0; j < matrix.height; j++) {
        ...
        if ((i < 7 && j < 7) || (i >= matrix.width - 7 && j < 7) || (i < 7 && j >= matrix.height - 7)) {
          continue;
        }

        // Skip drawing within the middle square
        if (i >= middleStart && i < middleEnd && j >= middleStart && j < middleEnd) {
          continue;
        }
