...
class Send3State extends State<Send3> {
  ...
  void createTransaction() async {
    ...
    try {
      var jsonRes = (await invoke("create_transaction", jsonEncode(input))).toString();
      print("JSON Response: $jsonRes");

      if (!mounted) return;

      if (jsonRes != null && jsonRes.trim().isNotEmpty) {
        try {
          var json = jsonDecode(jsonRes);
          print("create tx response: $json");

          calculateStandardFee(jsonRes);
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Error decoding JSON response. Please try again.")),
          );
        }
      } else {
        print("Error: Received empty or null JSON response");
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Received empty or null JSON response. Please try again.")),
        );
      }
    } catch (e) {
      print("Error invoking API: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error invoking API. Please check your connection and try again.")),
      );
    }
  }

  // Calculate a standard fee as returned by create_transaction with no fee specified
